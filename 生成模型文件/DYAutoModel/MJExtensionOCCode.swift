...
class MJExtensionOCCode: DyGenerateCode {

    
    override class func generateCode(config: DYCodeConfig) throws {
        let code = self.init();
        if code.config!.className.count == 0 ||  code.config!.path.count == 0 ||  code.config?.jsonStr.count == 0 {
        ...
        do {
            ...
            try code.generateFile(code.config!.className, "h", dict!);
            try code.generateFile(code.config!.className, "m", dict!);
        } catch {
            throw error;
        }
    }
            ...
            case is Array<Any>:
                ...
                for item in value as! [Any]{
                    ...
                    if item is [String : Any] {
                        let fileName = "\(self.config!.className)_\(key)";
